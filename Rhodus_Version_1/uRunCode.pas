...
type
  TRunFramework = class (TObject)
     ...
     function  compileCode (const src : string; bolShowAssembler : boolean) : boolean;
...
function TRunFramework.compileCode (const src : string;  bolShowAssembler : boolean) : boolean;
var i : integer;
begin
  result := True;
...
begin
   if compileCode (src, bolShowAssembler) then
      begin
