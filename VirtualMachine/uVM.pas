...
type
  ...
  private
    ...
    //function  popScalar: double;  // Not yet implemented
...
begin
  ...
  begin
    ...
    raise ERuntimeException.Create ('Stack underflow error in popoInteger');
end;
...
begin
  ...
  case symbol.symbolType of
    ...
    symUserFunc:  raise ERuntimeException.Create('User function: ' + inttostr(symbol.iValue));
  else
...
var
  ...
  nPureLocals : integer;
begin
  // Get the symbol table index to the function object
  index := popInteger;

  if module.symbolTable[index].symbolType = symUserFunc then
    begin
    funcRecord := module.symbolTable[index].fValue;
    ...
    // Initialize the purelocals to undefined so we can detect assignments to unassined variables
    ...
    raise ERuntimeException.Create('Call to symbol is not a user function: ' +
      module.symbolTable[index].symbolName);
end;
...
begin
  ...
  if variable.stackType <> stString then
    raise ERuntimeException.Create('left-hand side muyst be a string');
